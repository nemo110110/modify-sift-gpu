...
int	GlobalParam::       _timingS = 1;  //print out information of each step 每一步的信息
int	GlobalParam::       _timingO = 0;  //print out information of each octave每一组的信息
int	GlobalParam::       _timingL = 0;	//print out information of each level 每一层塔的信息
...
int GlobalParam::		_MaxFilterWidth = -1;	//maximum filter width, use when GPU is not good enough 最大滤波宽度
float GlobalParam::     _FilterWidthFactor	= 4.0f;	//the filter size will be _FilterWidthFactor*sigma*2+1  滤波宽度因子
float GlobalParam::     _DescriptorWindowFactor = 3.0f; //descriptor sampling window factor 描述器降采样窗口因子，这里可以做点文章！
int GlobalParam::		_SubpixelLocalization = 1; //sub-pixel and sub-scale localization 	亚像素和亚缩放本土化
int	GlobalParam::       _MaxOrientation = 2;	//whether we find multiple orientations for each feature  是否在每个特征值上发现多方向
int	GlobalParam::       _OrientationPack2 = 0;  //use one float to store two orientations 使用浮点数存储两个方向
float GlobalParam::		_MaxFeaturePercent = 0.005f;//at most 0.005 of all pixels  所有像素至多为0.005 最大特征百分比
int	GlobalParam::		_MaxLevelFeatureNum = 14096; //maximum number of features of a level  一个金字塔的最大特征值
int GlobalParam::		_FeatureTexBlock = 4; //feature texture storagte alignment  特征纹理存储对齐
