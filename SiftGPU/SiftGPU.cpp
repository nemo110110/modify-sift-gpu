...
int SiftGPU::RunSIFT()
{
	...
	if( _image_loaded ==0)
	{
		...
		GlobalUtil::StartTimer("Initialize Pyramid");
		//实例化金字塔，绑定纹理内存_featureTex，_orientationTex，_descriptorTex
		//                                                800* 600 *0.005
		//_down_sampled降采样因子
	_pyramid->InitPyramid(width, height, _texImage->_down_sampled);
		GlobalUtil::StopTimer();
...
{
	float	sa = _sigma0 * powf(2.0f, float(_level_min)/float(_dog_level_num)) ; // oct(s)
...
{ 
	//提供初始值
	if(_dog_level_num ==0) _dog_level_num = 3;  //差分金字塔3
	if(_level_max ==0) _level_max = _dog_level_num + 1; //max->	4
    if(_sigma0 ==0.0f) _sigma0 = 1.6f * powf(2.0f, 1.0f / _dog_level_num) ; //σ0=2.0158737
	if(_sigman == 0.0f) _sigman = 0.5f;  //σ=0.5


	_level_num = _level_max -_level_min + 1;  //6

	_level_ds  = _level_min + _dog_level_num; //2
	if(_level_ds > _level_max ) _level_ds = _level_max ;

	///
	float _sigmak = powf(2.0f, 1.0f / _dog_level_num) ; //σk=1.2599211
	float dsigma0 = _sigma0 * sqrt (1.0f - 1.0f / (_sigmak*_sigmak) ) ;//dσ0=1.2262735
	float sa, sb;

 
	sa = _sigma0 * powf(_sigmak, (float)_level_min) ;   //1.6
	sb = _sigman / powf(2.0f,   (float)GlobalUtil::_octave_min_default) ;//0.5

	_sigma_skip0 = sa>sb+ 0.001?sqrt(sa*sa - sb*sb): 0.0f;//σskip0=1.5198685

    sa = _sigma0 * powf(_sigmak, float(_level_min )) ; //1.6
    sb = _sigma0 * powf(_sigmak, float(_level_ds - _dog_level_num)) ;//1.6

	_sigma_skip1 = sa>sb + 0.001? sqrt(sa*sa - sb*sb): 0.0f;//σskip1=0

	_sigma_num = _level_max - _level_min; //σn=5  最大减去最小！
	_sigma = new float[_sigma_num]; //五个σ
	...
	//σ0=2.0158737
	//1.2262735,1.5450078,1.9465880,2.4525471,3.0900161
	//差分阈值
	if(_dog_threshold ==0)	_dog_threshold      = 0.02f / _dog_level_num ;//0.0066666664
	//边缘阈值
	if(_edge_threshold==0) _edge_threshold		= 10.0f;//10
}

void SiftGPUEX::MyDisplay(void (*UseDisplayShader)())
{
	...
	for(int i = 0; i<= 1; i++)
	{
		...
		dx = _texImage->GetImgWidth();
		dy = _texImage->GetImgHeight();
...
void SiftGPUEX::DisplaySIFT()
{
	...
	case 0:
		DisplayInput();  //显示图片
		DisplayFeatureBox(_sub_view); //画特征点
		break;

	case 1://我的
		//想等大小的图哈
		MyDisplay(ShaderMan::UseShaderDisplayGaussian);	
		break;
...
//	glTranslatef(0.0f, 0.0f, -1.0f);
	...
	{
		for(int j = 0; j < _dog_level_num; j++, idx++)  //极值点只在dog层
		{
			if(fnum[idx]>0)
			{
				if(view ==0)
				...
				}else
				{
					...
					//画四边形
					glColor3fv(_colors+ (idx%COLOR_NUM)*3);  //随机的颜色
