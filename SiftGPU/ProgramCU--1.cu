...
#define FILTERV_PIXEL_PER_THREAD 4                                   //每个线程滤波像素
#define FILTERV_TILE_WIDTH FILTERV_BLOCK_WIDTH          //滤波面宽度
#define FILTERV_TILE_HEIGHT (FILTERV_PIXEL_PER_THREAD * FILTERV_BLOCK_HEIGHT)   
																								//滤波面高度=每个线程滤波像素*垂直滤波线程块高度
...
#define KEY_BLOCK_DIMX (1<<KEY_BLOCK_LOG_DIMX)   //8
#define KEY_BLOCK_DIMY (1<<KEY_BLOCK_LOG_DIMY)    //8
...
#define DESCRIPTOR_COMPUTE_PER_BLOCK	4       //每个线程块描述计算
#define DESCRIPTOR_COMPUTE_BLOCK_SIZE	(16 * DESCRIPTOR_COMPUTE_PER_BLOCK)   //64
																					//线程块的大小描述计算

//一个块需要多少个关键点归一化
//how many keypoint descriptor to normalized in a block (32, ...)
#define DESCRIPTOR_NORMALIZ_PER_BLOCK	32   //归一化



///////////////////////////////////////////
//Thread block size for visualization                      可视化
//(This doesn't affect the speed of computation)
#define BLOCK_LOG_DIM 4
#define BLOCK_DIM (1 << BLOCK_LOG_DIM)  //16
...
template<int FW> __global__ void FilterH( float* d_result, int width)
{
	...
	const int HALF_WIDTH = FW >> 1;   //FM/2
	const int CACHE_WIDTH = FILTERH_TILE_WIDTH + FW -1;//共享内存大小，共享内存是一维的！！！
	const int CACHE_COUNT = 2 + (CACHE_WIDTH - 2)/ FILTERH_TILE_WIDTH;
	__shared__ float data[CACHE_WIDTH];
	const int bcol = IMUL(blockIdx.x, FILTERH_TILE_WIDTH);//blockIdx.x*128
	const int col =  bcol + threadIdx.x;//blockIdx.x*128+threadIdx.x 列
	const int index_min = IMUL(blockIdx.y, width);//这是二维的啊。1*800，第一行，总共600行，图像总大小800*600
	const int index_max = index_min + width - 1;//每一行的最大值
	int src_index = index_min + bcol - HALF_WIDTH + threadIdx.x; //这个是图像的索引！！！
...
#pragma unroll
	for(int j = 0; j < CACHE_COUNT; ++j)
	{
		...
		{
			int fetch_index = src_index < index_min? index_min : (src_index > index_max ? index_max : src_index); // 超过了就取极值
			data[cache_index] = tex1Dfetch(texData,fetch_index);//一维索引位置！
...
//LOG_SCALE拉普拉斯高斯缩放系数
//模板--下采样
// src_width 800   dst_width  400
template<int LOG_SCALE> __global__ void DownsampleKernel(float* d_result, int src_width, int dst_width)
{
	const int dst_col = IMUL(blockIdx.x, FILTERH_TILE_WIDTH) + threadIdx.x;//blockIdx.x *128+threadIdx.x
	if(dst_col >= dst_width) return;
	const int src_col = min((dst_col << LOG_SCALE), (src_width - 1));//乘以2  也就是0,2,4,6,8
	const int dst_row = blockIdx.y; //
	const int src_row = blockIdx.y << LOG_SCALE;   //乘以2  也就是0,2,4,6,8
...
{
	int src_width = src->GetImgWidth(), dst_width = dst->GetImgWidth() ;//dst:400   src:800
...
template<int FW> void ProgramCU::FilterImage(CuTexImage *dst, CuTexImage *src, CuTexImage* buf)
{
	...
	src->BindTexture(texData);
	dim3 gridh((width +  FILTERH_TILE_WIDTH - 1)/ FILTERH_TILE_WIDTH, height);//7*600
	dim3 blockh(FILTERH_TILE_WIDTH); //128
	FilterH<FW><<<gridh, blockh>>>((float*)buf->_cuData, width);//buf是指针会变化!
	CheckErrorCUDA("FilterH");                                                             //对原始影像src绑定到texData进行操作
																											 //并将结果保存到buf

	///vertical filtering  垂直滤波
	...
	FilterV<FW><<<gridv, blockv>>>((float*)dst->_cuData, width, height); //将buf绑定到texData进行操作
	CheckErrorCUDA("FilterV");                                                                           //并将结果保存到dst
}
...
void __global__ ComputeDOG_Kernel(float* d_dog, float2* d_got, int width, int height)
{
	...
	if(col < width && row < height) 
	{
		...
		d_dog[index] = v - vp; //啊，灰度值相减就是高斯差分啊！！！
		float vxn = tex1Dfetch(texC, index + 1); //next-->x
		float vxp = tex1Dfetch(texC, index - 1); //preview-->x
		float vyp = tex1Dfetch(texC, index - width);//next-->y
		float vyn = tex1Dfetch(texC, index + width);//preview-->y
		float dx = vxn - vxp, dy = vyn - vyp;
		float grd = 0.5f * sqrt(dx * dx  + dy * dy);//梯度
		float rot = (grd == 0.0f? 0.0f : atan2(dy, dx));
		d_got[index] = make_float2(grd, rot); // 这个什么梯度是什么鬼，只是C层的，也就是gus层的
	}
}
//计算DOG
...
{
       float data[3][3], v;   //data本层的9个
       float datap[3][3], datan[3][3];//datap上一层的9个像元。datan下一层的像元
#ifdef KEY_OFFSET_ONE  
       int row = (blockIdx.y << KEY_BLOCK_LOG_DIMY) + threadIdx.y + 1;//blockIdx.y*8+ threadIdx.y + 1 行
       int col = (blockIdx.x << KEY_BLOCK_LOG_DIMX) + threadIdx.x + 1;//blockIdx.x*8+ threadIdx.x + 1 列
#else
       int row = (blockIdx.y << KEY_BLOCK_LOG_DIMY) + threadIdx.y;//blockIdx.y*8+ threadIdx.y
       int col = (blockIdx.x << KEY_BLOCK_LOG_DIMX) + threadIdx.x;//blockIdx.x*8+ threadIdx.x
#endif
       int index = IMUL(row, width) + col;  //图像坐标
	   int idx[3] ={index - width, index, index + width}; //3个值，上中下
       int in_image =0;
       float nmax, nmin, result = 0.0f;
	   float dx = 0, dy = 0, ds = 0;  //求解方程！！！
...
#endif
       {
			...
			data[1][0] = tex1Dfetch(texC, idx[1] - 1);//左边像元
			data[1][2] = tex1Dfetch(texC, idx[1] + 1);//右边像元
			nmax = max(data[1][0], data[1][2]);//找到左右像元的最值！
			nmin = min(data[1][0], data[1][2]);

			if(v <=nmax && v >= nmin) goto key_finish;
			//if((v > nmax && v < 0 )|| (v < nmin && v > 0)) goto key_finish;
			READ_CMP_DOG_DATA(data[0], texC, idx[0]);  //第一行三个像元最值
			READ_CMP_DOG_DATA(data[2], texC, idx[2]);  //第三行三个像元最值
			//本层的极值！！！就是直接比较大小！！！

			//edge supression  边缘抑制   就是hessian矩阵
			float vx2 = v * 2.0f; //当前像元的2倍
			float fxx = data[1][0] + data[1][2] - vx2;//左侧加右侧减去中间的2倍
			float fyy = data[0][1] + data[2][1] - vx2;//上侧加下侧减去中间的2倍
			float fxy = 0.25f * (data[2][2] + data[0][0] - data[2][0] - data[0][2]);
			float temp1 = fxx * fyy - fxy * fxy;          //tr
			float temp2 = (fxx + fyy) * (fxx + fyy);  //det
			if(temp1 <=0 || temp2 > edge_threshold * temp1) goto key_finish;


			//read the previous level //读取之前的一层高斯差分
			READ_CMP_DOG_DATA(datap[0], texP, idx[0]);
			READ_CMP_DOG_DATA(datap[1], texP, idx[1]);
			READ_CMP_DOG_DATA(datap[2], texP, idx[2]); //找出这一层的最大值


			//read the next level 读取之后的一层高斯差分
			READ_CMP_DOG_DATA(datan[0], texN, idx[0]);
			READ_CMP_DOG_DATA(datan[1], texN, idx[1]);
			READ_CMP_DOG_DATA(datan[2], texN, idx[2]);//找出这一层的最大值
			
			if(subpixel_localization)
			{
				//我突然明白了，这个是权重！！！
				//subpixel localization  亚像素定位
				float fx = 0.5f * (data[1][2] - data[1][0]);    //右像元减去左像元/2
				float fy = 0.5f * (data[2][1] - data[0][1]);   //下像元减去上像元/2
				float fs = 0.5f * (datan[1][1] - datap[1][1]);//上层像元减去下层像元/2
				//6个最邻近的！！！

				float fss = (datan[1][1] + datap[1][1] - vx2); //左侧加右侧减去中间的2倍
				float fxs = 0.25f* (datan[1][2] + datap[1][0] - datan[1][0] - datap[1][2]);//x方向
				float fys = 0.25f* (datan[2][1] + datap[0][1] - datan[0][1] - datap[2][1]);//y方向
...
key_finish:
       if(in_image) d_key[index] = make_float4(result, dx, dy, ds);//四维向量！！！
}
...
void ProgramCU::ComputeKEY(CuTexImage* dog, CuTexImage* key, float Tdog, float Tedge)
{
	...
	CuTexImage* dogp = dog - 1;  //dogpreview之前的一层高斯差分
	CuTexImage* dogn = dog + 1; //dognext之后的一层高斯差分
#ifdef KEY_OFFSET_ONE
	...
	dogp->BindTexture(texP);//之前的一层高斯差分
	dog ->BindTexture(texC);//中间的一层高斯差分
	dogn->BindTexture(texN);//之后的一层高斯差分
...
#pragma unroll
				for(int i = 0; i < 4 ; ++i, ++scol)
				{
					...
					v[i] = (scol < ws -1 && scol > 0 && temp.x!=0) ? 1 : 0;  //不是1就是0！！！
				}
			}
			hist[hidx] = make_int4(v[0], v[1], v[2], v[3]); //四维向量！！！保存了什么？？？
...
void __global__ MultiplyDescriptor_Kernel(int* d_result, int num1, int num2, int3* d_temp)
{
	...
	//idx1=blockIdx.y*blockDim.y+threadIdx.y
