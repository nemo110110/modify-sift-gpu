...
{
public:
	float*		_sigma;  //金字塔每一层的σ
	float		_sigma_skip0; // ？
	float		_sigma_skip1; //？
	
	//sigma of the first level
	float		_sigma0;  //金字塔第0层的σ
	float		_sigman;
	int			_sigma_num; //金字塔层数-1

	//how many dog_level in an octave
	int			_dog_level_num;   //一个octave里面有几个高斯差分金字塔
	int			_level_num;    //总共有几层金字塔
	...
	int			_level_ds;
	//dog threshold
	float		_dog_threshold;  //高斯差分阈值
	//edge elimination
	float		_edge_threshold;//边缘阈值
	void		 ParseSiftParam();
public:
...
class SiftGPUEX:public SiftGPU
{
	...
	void MyDisplay(void (*UseDisplayShader)());		//display all images in one octave
...
public:
	...
	SIFTGPU_EXPORT SiftMatchGPU(int max_sift = 14096);
	...
	SIFTGPU_EXPORT virtual int  GetSiftMatch(
				...
				int mutual_best_match = 1); //mutual best match or one way   相互最佳匹配或单向匹配

	//two functions for guded matching, two constraints can be used  引导匹配用到了两个函数，两个约束
	//one homography and one fundamental matrix, the use is as follows 一个基础矩阵，一个单应矩阵
	//1. for each image, first call SetDescriptor then call SetFeatureLocation 每个图像，先执行SetDescriptor再执行SetFeatureLocation 
	//2. Call GetGuidedSiftMatch
	//input feature location is a vector of [float x, float y, float skip[gap]]  输入的特征位置是一个向量（x，y，）
...
SIFTGPU_EXPORT_EXTERN SiftMatchGPU* CreateNewSiftMatchGPU(int max_sift = 14096);
