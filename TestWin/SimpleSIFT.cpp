...
int main()
{
    ...
    if(hsiftgpu == NULL) return 0;
        ...
        SiftGPU* sift = pCreateNewSiftGPU(1);
							//printf("2.12\n");
        SiftMatchGPU* matcher = pCreateNewSiftMatchGPU(14096);
							//printf("2.13\n");
    #endif
//——————————————————————————
				//printf("3");
    ...
    SiftMatchGPU *matcher = new SiftMatchGPU(14096);
#endif
    ...
    char * argv[] = {"-cuda"," -fo", "-1",  "-v", "1"};//
    //-fo -1    staring from -1 octave 
    //-v 1      only print out # feature and overall time  显示总时间
	...
	//          Checkout src\TestWin\MultiThreadSIFT  这应该是多显卡，不看了
    /////////////////////////////////////////////////////////////////////////////////////
	...
	//使用SetMaxDimension这个和"-maxd"自己的显卡
    ...
    if(sift->RunSIFT("d:/DSC2_2048.jpg"))
    {
				//printf("7");
        //Call SaveSIFT to save result to file, the format is the same as Lowe's
        sift->SaveSIFT("d:/DSC2_2048.sift"); //Note that saving ASCII format is slow
    ...
    if(sift->RunSIFT("d:/DSC1_2048.jpg"))
    {

		sift->SaveSIFT("d:/DSC1_2048.sift"); 
    ...
    int (*match_buf)[2] = new int[num1][2];//在这里是较大值！
	...
	vector<SiftGPU::SiftKeypoint> key1(num_match), key2(num_match);    
    for(int i  = 0; i < num_match; ++i)
    {
        //How to get the feature matches: 
        key1[i] = keys1[match_buf[i][0]]; //image1中的match
        key2[i] = keys2[match_buf[i][1]]; //image2中的match
